...
s3d.on("show", function(event) {
    if(settingsArray['Settings.Gamepad'] == 1){
        $('#exit .button').attr('src','s3d://assets/buttons/'+settingsArray['Game.IconSet']+'_Y.png');
...
s3d.on('controllerinput', function(e) {
    if(settingsArray['Settings.Gamepad'] == 1){
        if (e.data.Y == 1) { s3d.command('game.exit')};
		if (e.data.B == 1) { s3d.hide(); };
    }
});