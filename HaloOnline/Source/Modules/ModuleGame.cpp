...
#include "Bungie\BungieGraphics.hpp"
...
namespace
{
	...
	bool CommandGameInfo(const std::vector<std::string>& Arguments, std::string& returnInfo)
	{
		...
		ss << std::hex << "ThreadLocalStorage: 0x" << std::hex << (size_t)(void*)HaloOnline::GetMainTls() << std::endl;
		...
		ss << "Players global addr: 0x" << std::hex << HaloOnline::GetMainTls(0x40).Read<uint32_t>() << std::endl;
	...
	bool CommandGameForceLoad(const std::vector<std::string>& Arguments, std::string& returnInfo)
	{
		...
		mapName = HaloOnline::Instance().GetMapsFolder() + mapName;
	...
	{
		// Open the .map file
		auto mapPath = HaloOnline::Instance().GetMapsFolder() + mapName + ".map";
...
namespace Modules
{
	ModuleGame::ModuleGame() : ModuleBase("Game")
	{
		...
		VarSkipTitleSplash = AddVariableInt("SkipTitleSplash", "titlesplash", "Skip the HaloOnline splash screen and go straight to the main menu", eCommandFlagsArchived, 1);
		...
		/*EXAMPLES: adds a variable "Game.Name", default value HaloOnline, calls VariableGameNameUpdate when value is updated
		AddVariableString("Name", "gamename", "Title of the game", "HaloOnline", VariableGameNameUpdate);
	...
	{
		auto searchPath = HaloOnline::Instance().GetMapsFolder() + "*.map";
