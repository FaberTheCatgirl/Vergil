...
namespace
{
	bool VariableSaturationUpdate(const std::vector<std::string>& Arguments, std::string& returnInfo)
	{
		...
		Pointer hueSaturationControlPtr(HaloOnline::GetMainTls(GameGlobals::Graphics::TLSOffset)[0]);
	...
	bool VariableBloomUpdate(const std::vector<std::string>& Arguments, std::string& returnInfo)
	{
		...
		Pointer atmoFogGlobalsPtr(HaloOnline::GetMainTls(GameGlobals::Bloom::TLSOffset)[0]);
	...
	bool VariableDepthOfFieldUpdate(const std::vector<std::string>& Arguments, std::string& returnInfo)
	{
		...
		Pointer dofGlobals(HaloOnline::GetMainTls(GameGlobals::DepthOfField::TLSOffset)[0]);
	...
	bool VariableLetterboxUpdate(const std::vector<std::string>& Arguments, std::string& returnInfo)
	{
		...
		Pointer cinematicGlobals(HaloOnline::GetMainTls(GameGlobals::Cinematic::TLSOffset)[0]);
	...
	bool VariableUIScalingUpdate(const std::vector<std::string>& Arguments, std::string& returnInfo)
	{
		...
		else if(HaloOnline::Instance().GameHasMenuShown) //If the user comes to their senses and turns UI scaling back on then apply the change. Running before the main menu has shown may result in a crash.
