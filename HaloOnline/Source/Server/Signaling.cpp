...
namespace Server::Signaling
{
	void Initialize()
	{
		...
		wspsender = RegisterPacket<WebSocketPacketData>("haloonline-signal-server-echo", wsphandler);
	...
	void StartServer()
	{
		...
		{
			Modules::ModuleUPnP::Instance().UPnPForwardPort(true, port, port, "HaloOnline Signal Server");
		}
		if (!is_listening)
