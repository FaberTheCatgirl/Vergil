...
namespace
{
	...
	void GetEndpoints()
	{
		...
		if (!in || !in.is_open())
		{
			Utils::Logger::Instance().Log(Utils::LogTypes::Network, Utils::LogLevel::Error, "Server::ReportHandler::GetEndpoints(): Failed to open \"%s\". LastError: %d", haloonlineJsonPath, GetLastError());
			return;
		}
		...
		if (json.Parse<0>(ss.str().c_str()).HasParseError())
		{
			...
			Utils::Logger::Instance().Log(Utils::LogTypes::Network, Utils::LogLevel::Error, "Server::ReportHandler::GetEndpoints(): Failed to parse \"%s\": %s", haloonlineJsonPath, error);
			return;
		}
