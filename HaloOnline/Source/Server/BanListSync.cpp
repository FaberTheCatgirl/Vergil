...
namespace
{
	// retrieves player report endpoints from haloonline.json
	bool GetEndpoints()
	{
		const auto haloonlineJsonPath = "mods/haloonline.json";

		std::ifstream in(haloonlineJsonPath, std::ios::in | std::ios::binary);
		if (!in || !in.is_open())
		{
			Utils::Logger::Instance().Log(Utils::LogTypes::Network, Utils::LogLevel::Error, "Server::BanListSync::GetEndpoints(): Failed to open \"%s\". LastError: %d", haloonlineJsonPath, GetLastError());
			return false;
		}
		...
		if (json.Parse<0>(ss.str().c_str()).HasParseError())
		{
			...
			Utils::Logger::Instance().Log(Utils::LogTypes::Network, Utils::LogLevel::Error, "Server::BanListSync::GetEndpoints(): Failed to parse \"%s\": %s", haloonlineJsonPath, error);
			return false;
		}
		...
		if (json.IsObject() && json.HasMember("banlistUrls"))
		{
			...
			if (!urls.IsArray())
			{
				Utils::Logger::Instance().Log(Utils::LogTypes::Network, Utils::LogLevel::Error, "Server::BanListSync::GetEndpoints(): \"banlistUrls\" must be an array \"%s\"", haloonlineJsonPath);
				return false;
			}			
