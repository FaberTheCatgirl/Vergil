...
BOOL InitInstance(HINSTANCE hModule)
{
	...
	//Make sure our working directory is in the Halo Online root folder
	char appPath[MAX_PATH];
	GetModuleFileNameA(hModule, appPath, MAX_PATH);
	std::string HaloOnlinePath = std::string(appPath);
	HaloOnlinePath = HaloOnlinePath.substr(0, HaloOnlinePath.find_last_of("\\"));
	SetCurrentDirectoryA(HaloOnlinePath.c_str());
	...
	HaloOnline::SetMainThreadID(GetCurrentThreadId());

	Patches::ApplyRequired();
	HaloOnline::Instance().Initialize();
...
// TODO: remove this export, removing it needs us to use a new haloonline-enabled eldorado.exe though
DWORD GetAdaptersInfo(PIP_ADAPTER_INFO pAdapterInfo, PULONG pOutBufLen)
{
	return 0;
}

int GetHaloOnlineVersion()
{
