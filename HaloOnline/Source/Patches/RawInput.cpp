...
namespace
{
	...
	bool RawInputHookImpl(RAWINPUT *rwInput)
	{
		...
		Pointer InputPtr = HaloOnline::GetMainTls(GameGlobals::Input::TLSOffset)[0];
		if (!InputPtr)
			return true;
		...
		Pointer SettingsPtr = HaloOnline::GetMainTls(GameGlobals::GameSettings::TLSOffset)[0];
		...
		Pointer PlayerData = HaloOnline::GetMainTls(GameGlobals::PlayerAlt::TLSOffset)[0];
		if (!PlayerData)
			return true;
