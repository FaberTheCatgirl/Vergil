...
namespace
{
	...
	void preferences_set_defaults_hook()
	{
		...
		Blam::Preferences *preferences = HaloOnline::GetMainTls(0x18)[0];
		...
		*(uint32_t *)HaloOnline::GetMainTls(0x18)[0](0x8400C) = 1;
	}
	...
	void __fastcall campaign_scoring_sub_6E59A0(char *scoreboard, void *, Blam::DatumHandle handle, Blam::Events::EventType event_type, short a4, Blam::ePlayerStatType player_stat_type, char a6)
	{
		...
		*((short *)scoreboard_unknown + 2) = v8;
		...
		if (a6)
		{
			...
			{
				auto player_datum = (Blam::Players::PlayerDatum*)data_array_sub_55B710(HaloOnline::Instance().GetMainTls(0x40).Read<Blam::DataArray<Blam::Players::PlayerDatum>*>(), handle);
	...
	void __cdecl sub_6948C0_hook(int a1)
	{
		...
		auto *tls = (Blam::Memory::tls_data *)HaloOnline::Instance().GetMainTls();
	...
	void dirty_disk_error_debug()
	{
		...
		if (!HaloOnline::Instance().IsDedicated())
		{
...
namespace
{
	...
	{
		// "There once was a man from Blam...... Nothing rhymes with Blam.but he got a dirty disc error for the Alpha or Profile Build and exploded.  The End."
		__asm
		{
	...
	std::vector<Blam::e_secondary_skull> secondary_skulls
	{
		...
		{ Blam::e_secondary_skull::_grunt_birthday_party, false },
		{ Blam::e_secondary_skull::_iwhbyd, false },
