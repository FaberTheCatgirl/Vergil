...
namespace
{
	...
	void UpdateMapModifier()
	{
		...
		if (mapModifierObjectIndex == -1 || (mapModifierState.WaitingForNewRound && roundInProgress))
		{
			...
			if (mapModifierState.IsActive)
			{
				...
				{
					auto rasterizerGameStates = (uint8_t*)HaloOnline::GetMainTls(0x3bc)[0];
	...
	void ResetRuntime()
	{
		...
		auto runtime = (c_game_engine_object_runtime_state*)((uint8_t*)HaloOnline::GetMainTls(0x48)[0] + 0x1059c);
