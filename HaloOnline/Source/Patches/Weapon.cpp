...
namespace
{
	...
	bool UnitIsDualWielding(Bungie::DatumHandle unitHandle)
	{
		if (!unitHandle)
			return false;
		auto objectHeaderArrayPtr = HaloOnline::GetMainTls(GameGlobals::ObjectHeader::TLSOffset)[0];
	...
	uint32_t GetObjectDataAddress(uint32_t objectDatum)
	{
		...
		Pointer objectHeaderPtr = HaloOnline::GetMainTls(GameGlobals::ObjectHeader::TLSOffset)[0];
