...
size_t HaloOnline::MainThreadID = 0;

HaloOnline::HaloOnline()
{
}

bool(__cdecl * HaloOnline::Video_InitD3D)(bool, bool) = (bool(__cdecl *) (bool, bool)) 0xA21B40;

bool __cdecl HaloOnline::hooked_Video_InitD3D(bool windowless, bool nullRefDevice) {
	// TEMP: leave window enabled for now so async networkWndProc stuff still works
	return (*Video_InitD3D)(false, nullRefDevice);
}

void HaloOnline::killProcessByName(const char *filename, int ourProcessID)
{
...
void HaloOnline::Initialize()
{
	...
	// If instancing is enabled then load the instanced haloonline_prefs.cfg
	if (instanceName != "")
	{
		std::stringstream keystream;
		keystream << "Execute haloonline_prefs_" << instanceName << ".cfg";
		Modules::CommandMap::Instance().ExecuteCommand(keystream.str());

		std::stringstream ss;
		ss << "Execute \"" << haloonline_appdata << "\\keys_" << instanceName << ".cfg\"";
		Modules::CommandMap::Instance().ExecuteCommand(ss.str());
	}
	else
	{
		std::stringstream ss;
		ss << "Execute haloonline_prefs.cfg";
		Modules::CommandMap::Instance().ExecuteCommand(ss.str());

		std::stringstream keystream;
		keystream << "Execute \"" << haloonline_appdata << "\\keys.cfg\"";
		Modules::CommandMap::Instance().ExecuteCommand(keystream.str());
	}
...
std::string HaloOnline::GetDirectory()
{
	char Path[MAX_PATH];
	HMODULE hMod;
...
void HaloOnline::OnMainMenuShown()
{
	if (GameHasMenuShown)
		return;
...
//void HaloOnline::setWatermarkText(const std::string& Message)
//{
...
void* _mainTLS;
Pointer HaloOnline::GetMainTls(size_t tlsOffset)
{
