...
namespace
{
	...
	void UpdateForgeSky(Forge::ForgeMapModifierProperties *mapModifierProperties)
	{
		...
		if (desiredSkyIndex != forgeSkyState.Index)
		{
			...
			auto screenEffects = ElDorito::GetMainTls(0x338).Read<Bungie::DataArray<ScreenEffectDatum>*>();
			...
			auto loopingSounds = ElDorito::GetMainTls(0xCC).Read<Bungie::DataArray<s_object_looping_sound>*>();
	...
	float __fastcall LightIlluminationFunction(uint8_t *thisptr, uint32_t lightDatumIndex, float a1, float a2)
	{
		...
		auto& lightArray = ElDorito::GetMainTls(0x498)[0].Read<Bungie::DataArray<LightDatum>>();
	...
	void ScreenEffectsHook(RealVector3D *a1, RealVector3D *a2, ScreenEffectData *renderData, void *a4, int localPlayerIndex)
	{
		...
		*(DWORD *)((uint8_t*)a4 + 0x120) = 0;
		...
		auto &screenEffectDatumArray = ElDorito::GetMainTls(0x338)[0].Read<Bungie::DataArray<ScreenEffectDatum>>();
